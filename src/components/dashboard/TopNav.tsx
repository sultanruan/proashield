...
export function TopNav({ fullName, username, variant = 'exec' }: TopNavProps) {
  ...
  return (
    <header className="h-14 bg-white border-b border-[#E4E4E7] flex items-center px-6 gap-4 shrink-0">
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <Link
          href={variant === 'seller' ? '/dashboard/seller' : '/dashboard/exec'}
        ...
        {variant === 'seller' ? (
          <span className="text-xs text-[#A1A1AA] font-medium">Seller Dashboard</span>
        ) : (
